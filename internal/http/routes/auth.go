...
func RegisterAuthRoutes(router *mux.Router) {
	...
	auth.Handle("/logout", middleware.AuthMiddleware(http.HandlerFunc(handlers.Logout))).Methods("POST")
	auth.HandleFunc("/verify", handlers.VerifyEmail).Methods("GET")
	auth.Handle("/resend-verification", middleware.AuthMiddleware(http.HandlerFunc(handlers.ResendVerification))).Methods("POST")
